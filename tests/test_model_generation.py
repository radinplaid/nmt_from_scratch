    ...
    def test_generate_with_enc_output(self, model, sample_src):
        ...
        with torch.no_grad():
            ...
            result2 = model.generate(
                sample_src, max_len=10, bos_id=2, eos_id=3, enc_output=enc_output
            )
...
class TestBeamSearchMethod:
    ...
    def test_beam_search_returns_tensor(self, model, sample_src):
        ...
        result = model.beam_search(
            sample_src, max_len=10, beam_size=3, bos_id=2, eos_id=3
        )
        assert isinstance(result, torch.Tensor)
    ...
    def test_beam_search_respects_max_len(self, model, sample_src):
        ...
        max_len = 5
        result = model.beam_search(
            sample_src, max_len=max_len, beam_size=3, bos_id=2, eos_id=3
        )
    ...
    def test_beam_search_different_beam_sizes(self, model, sample_src):
        ...
        for beam_size in [1, 2, 3, 5]:
            result = model.beam_search(
                sample_src, max_len=10, beam_size=beam_size, bos_id=2, eos_id=3
            )
    ...
    def test_beam_search_deterministic(self, model, sample_src):
        ...
        result1 = model.beam_search(
            sample_src, max_len=10, beam_size=3, bos_id=2, eos_id=3
        )

        torch.manual_seed(42)
        result2 = model.beam_search(
            sample_src, max_len=10, beam_size=3, bos_id=2, eos_id=3
        )
...
class TestCompareGreedyVsBeamSearch:
    ...
    def test_beam_size_1_equals_greedy(self, model):
        ...
        with torch.no_grad():
            ...
            beam_result = model.beam_search(
                src, max_len=10, beam_size=1, bos_id=2, eos_id=3
            )
